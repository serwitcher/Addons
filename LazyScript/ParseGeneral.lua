lazyScript.metadata:updateRevisionFromKeyword("$Revision: 760 $")
...
function lazyScript.loadActionTable()
   ...
   lazyScript.actions.cannibalize =
      lazyScript.Action:New("cannibalize", "Ability_Racial_Cannibalize", false, true, true)
...
function lazyScript.bitParsers.slotFreeAction(bit, actions, masks)
   ...
   if (not lazyScript.otherActions[thisAction .. "-free"]) then
      lazyScript.otherActions[thisAction .. "-free"] =
         lazyScript.Action:New(thisAction, nil, nil, false, nil, thisAction)
   end
...
function lazyScript.bitParsers.sayIn(bit, actions, masks)
   ...
   if
      (channel == "Guild" or channel == "Party" or channel == "Raid" or channel == "Say" or channel == "Emote" or
         channel == "RAID_WARNING" or
         channel == "Yell")
    then
...
function lazyScript.bitParsers.applyWeaponBuff(bit, actions, masks)
   ...
   if (not lazyScript.applyWeaponBuffActions[key]) then
      lazyScript.applyWeaponBuffActions[key] =
         lazyScript.ApplyWeaponBuff:New("applyWeaponBuff" .. equipSlot, weaponBuffName, equipSlot)
   end
...
function lazyScript.bitParsers.ifTargetInMeleeRange(bit, actions, masks)
   ...
   elseif (not lazyScript.rangeCheckAction) then
      ...
      lazyScript.p(
         "ifTargetInMeleeRange is not currently supported for your class because you do not have " ..
            lazyScript.safeString(rangeAction.name) .. " yet."
      )
      return nil
   end
...
function DTMH3(unitId, wantPct, wantDeficit, sayNothing)
   --=================================================
   -- Dadi changes -----------------------------------
   --=================================================
   local currentHP_MH, maxHP_MH, found_MH = MobHealth3:GetUnitHealth(unitId)
   local pctMH = 0
   local deficitHP_MH = 0
   if currentHP_MH and maxHP_MH then
      pctMH = (currentHP_MH / maxHP_MH) * 100
      deficitHP_MH = maxHP_MH - currentHP_MH
   end
   local wnt = wantPct and "true" or "false"
   --print("--= DTMH3 =-->:" .. tostring(pctMH))
   ...
   elseif (unitId == "target") then
      if (wantPct) then
         ...
         if (not currentHP_MH) then
            return nil
         end
         if (wantDeficit) then
            if not maxHP_MH then
               return nil
            end
            return currentHP_MH
         else
...
function lazyScript.masks.GetUnitHealth(unitId, wantPct, wantDeficit, sayNothing)
   local ret = DTMH3(unitId, wantPct, wantDeficit, sayNothing)
   --print("--= DTMH3 =-->: " .. tostring(ret))
   return ret
   --[[
if (unitId == "player" or unitId == "pet" or UnitPlayerOrPetInParty(unitId) or UnitPlayerOrPetInRaid(unitId)) then
      if (wantPct) then
...
function lazyScript.masks.PlayerMountedIndex(sayNothing)
   ...
   while true do
      ...
      elseif
         (string.find(texture, "Ability_Mount_") or string.find(texture, "Spell_Nature_Swiftness$") or
            string.find(texture, "INV_Misc_QirajiCrystal$") or
            string.find(texture, "INV_Misc_Foot_Kodo$"))
       then
         mountBuffIndex = buffIndex
         break
      end
...
function lazyScript.masks.IsGlobalCooldown(gcdActionObj)
   return function(sayNothing)
      ...
      if not slot then
         if not sayNothing then
            lazyScript.p(
               "To enable checking of the Global Cooldown, please place the " ..
                  lazyScript.safeString(gcdActionObj.name) .. " action on your action bar."
            )
         end
         return nil
      end
   ...
   if unitId == "target" then
      return function(sayNothing)
         if UnitIsFriend(unitId, "player") then
            return lazyScript.masks.HasBuffOrDebuff(
               unitId,
               "buff",
               buffObj.texture,
               buffObj.name,
...
function lazyScript.masks.IsImmune(action)
   return function(sayNothing)
      if
         (lazyScript.perPlayerConf.Immunities[action] and
            lazyScript.perPlayerConf.Immunities[action][UnitName("target")])
       then
...
function lazyScript.validateUnitId(unitId)
   for _, validUnitId in ipairs(lazyScript.VALID_UNIT_IDS) do
      ...
      if s then
         ...
         if
            n and
               not (((base == "party" or base == "partypet") and (n >= 1 and n <= 4)) or
                  ((base == "raid" or base == "raidpet") and (n >= 1 and n <= 40)))
          then
            return false
         end
